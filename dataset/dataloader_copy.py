...
class DataLoad(Dataset):
	...
	def __getitem__(self, idx):
		...
		for device in self.params.device_list:
			...
			depth_range_mask = (importers.image.load_depth(datum[device]["depth"], scale=self.params.depth_scale) < self.params.depth_threshold).float() ##zzzzz change scale to 1 from original 0.001
			depth_img = importers.image.load_depth(datum[device]["depth"], scale=self.params.depth_scale) * depth_range_mask
