...
class DataLoaderParams:
	...
	,root_path 
	,device_list
	,decimation_scale = 1 #zzz change from 2 to 1
	,device_repository_path = "."
	,depth_scale = 0.0002 #zzz change from 0.001 to 1
	,depth_threshold = 5):
...
class DataLoad(Dataset):
	def __init__(self, params):
		...
		for recording in os.listdir(root_path):
			...
			for file in os.listdir(data_path):
				...
				_id,_name,_type = filename.split("_")
	...
	def __getitem__(self, idx):
		...
		for device in self.params.device_list:
			#print(device)
			color_img = importers.image.crop_image(datum[device]["color"])
			depth_range_mask = (importers.image.crop_depth(datum[device]["depth"], scale=self.params.depth_scale) < self.params.depth_threshold).float() ##zzzzz change scale to 1 from original 0.001
			depth_img = importers.image.crop_depth(datum[device]["depth"], scale=self.params.depth_scale) * depth_range_mask
			gt_img = importers.image.crop_depth(datum[device]["gt"], scale=self.params.depth_scale) * depth_range_mask
