...
def depth_GT(inout):
    i = iter(inout.keys())
    key = next(i)
    loss, loss_map = inverse_huber_loss_masked(inout[key]["depth"]["gt"], \
        inout[key]["depth"]["prediction"], inout[key]["depth"]["mask"],\
        inout[key]["depth"]["count"])
    for k in i:
        loss_tuple = inverse_huber_loss_masked(inout[k]["depth"]["gt"],\
            inout[k]["depth"]["prediction"], inout[k]["depth"]["mask"],\
            inout[k]["depth"]["count"])
        loss += loss_tuple[0]
        loss_map += loss_tuple[1]
...
def robust_photometric_supervision_splat(inout, kernel_size=5, std=1.5, alpha=0.85, mode='gaussian'):
    ...
    for k in i:
        ...
        count = torch.sum(mask)
        print ("=========In losses ", k," has ",count,"valid depth values in splatted image")
        masked_color = inout[k]["color"]["original"] * mask
        l1_l_map_k = mask * torch.abs(\
            masked_color - inout[k]["color"]["splatted"])
        l1_l_map_k = L1_Charbonnier_premasked(l1_l_map_k, epsilon_squared=0.00001)
        l1_l_map += l1_l_map_k
